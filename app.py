...
def logout():
    ...
    print("[LOGOUT] Route called", file=sys.stderr)
    data = request.json
    session_id = data.get("session_id")
    print(f"[LOGOUT] Received session_id: {session_id}", file=sys.stderr)
    if not session_id:
        print("[LOGOUT] ERROR: Missing session_id", file=sys.stderr)
    ...
    try:
        ...
        if not row:
            print(
                f"[LOGOUT] ERROR: Session not found for session_id {session_id}",
                file=sys.stderr,
            )
            cur.close()
            conn.close()
        ...
        print(
            f"[LOGOUT] Updating session {session_id} with endtime {endtime.isoformat()} and duration {duration}",
            file=sys.stderr,
        )
        cur.execute(
        ...
        conn.commit()
        print(f"[LOGOUT] DB update committed for session {session_id}", file=sys.stderr)
        cur.close()
        conn.close()

        print(f"[LOGOUT] Success for session {session_id}", file=sys.stderr)
        return jsonify(
            {
                "success": True,
        ...
        print(f"[LOGOUT] EXCEPTION: {str(e)}", file=sys.stderr)
        traceback.print_exc()
        return (
